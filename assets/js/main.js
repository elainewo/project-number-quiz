...
for (let index = 0; index < data.length; index++) {
    ...
    frage.innerText = element.question;
    divContent.appendChild(frage);

    element.choice.forEach(x => {
        let wahl = document.createElement("span");
        wahl.setAttribute("class", "newSpan")
        wahl.innerText += `${x}`;
        console.log(wahl.innerText);
        divContent.appendChild(wahl);
        // Die Methode mit Funtkion funktioniert nicht..
        // function auswahl() {
        //     if (wahl.addEventListener("click") && wahl.innerText == "Nevada") {
        //         console.log("helooo")
        //     }
        //     element.choice.auswahl()
        // }
    })
}

// Ich möchte alle <span> Elemente mit addEventListener "click" ansprechen. Damit ich danach eine 
//If-Kondition aufstellen kann, bei einer Antwortbestätigung mit dem Klick auf den richtigen 
//<span>content</span>

console.log(document.getElementsByClassName("newSpan"))

// Das ansprechen mit querySelectorAll funktioniert auch nicht..
